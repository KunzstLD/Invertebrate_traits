...
For the \textit{sim\_base} scenario the trait aggregation methods based on the mean result in the same range of aggregated trait affinities irrespective of the level of trait variation. Aggregation methods using the median yielded greater ranges of aggregated trait affinities than methods using the mean. Thereby, the \textit(stepwise\_agg \textsubscript{median}) yielded the greatest range of aggregated trait affinities. 

By contrast, the ranges of trait affinities differed for all aggregation methods for the simulation scenarios \textit{sim\_extreme} and \textit{sim\_variation}. The greatest ranges of trait affinities were produced by the \textit{stepwise\_agg \textsubscript{median}}. With increasing level of trait variation the \textit{stepwise\_agg \textsubscript{mean}} produced a wider range of affinities than the \textit{direct\_agg\textsubscript{mean}} and \textit{weighted\_agg}.  
% ? direct median and stepwise median similar range
