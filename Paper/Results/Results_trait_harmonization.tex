...
% Hence, the interpretation of the trait composition is the same as when only using harmonized grouping features. 
...
% PUP comment: 
% What are exactly „RDA species scores“? The coordinates of species along the first factorial axis in RDA?
% If „yes“, I’m not sure this result presentation is suitable. In fact the box-plots summarizing the distributions of the set of species in the RDA analysis, after applying different strategies of trait description, can be very similar (it’s not surprising, because in each case the mean coordinate of species is 0), but the individual locations of species along each axis can be very different from an RDA to another axis (i.e. from a grouping feature definition strategy to another). It would be probably far more interesting to represent the distribution of deviations between the RDA scores provided by each strategy and the original RDA scores (from Szöcs et al., 2018)..
