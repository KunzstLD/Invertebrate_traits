...
% Start general
% Why do we need trait-based analysis, what's the problem with the state of data coverage
% Implications of trait definitions chapter

% Keine Überscrhiften kopieren: sollte eher eine generelle Aussage haben
\subsection*{Differences in trait affinities obtained by trait aggregation methods compared to traits assigned at family-level}

% Hier erstmal ein Satz allgemeiner Einstieg
% Z.B.: We analysed the difference between aggregated and traits assigned by experts for two data sets on..
...
% Those cases...R: schwierig einzuordnen - waren das viele - was sind das für fälle
...
% R: Unverständlich - kann auch sein, weil ich die neue Sodom nebenbei höre - aber soll das erklären, warum der impact minor war - oder erklärt das noch mal die Ergebnisse
...
% R: Steht das nicht schon im zweiten Satz? True rm!
% Klarstellen was gemacht wurde 
Instead, using the mean or the median changed the distribution of differences in trait affinities between aggregated and assigned traits. The minor impact on trait aggregation of the weightings may be explained by the fact that a considerable portion of taxa had low numbers of genera or species entries. % R: Too much precision with the numbers
...
% R: OK - hier soll einfach gesagt werden, dass bei 0 1 Codierung die Differenzen auch größer werden. Macht Sinn, kann man so schreiben - sollte aber auch stärker mit dem vorherigen Abschnitt verknüpft werden. The differences between aggregated and assigned traits (kann man das nicht irgendwo kürzer und eleganter beschreiben?) depended on the coding. The database with .. coding had the highest..
...
% R: Könnte man beides erwähnen - aber insgesamt würde ich nicht zu viele Sachen reinpacken, sonst mäandert es so sehr. Aber ein Hinweis, dass es bei großskaligen Analysen vielleicht auch wichtiger ist usw. wegen klimatischer Unterschiede - wobei die ja eher dann auch intra-species variieren und das ja gar nicht viel mit dem aktuellen Vergleich der aggregationsmethoden zu tun hat. Im Prinzip gehört das eher in einen einleitenden Abschnitt zu traits allgemeiner
\textit{\color{blue} Could mention here: \begin{itemize}
  \item Intra-species variation of some traits (voltinism varies along climatic gradients)
  \item Obtaining trait information on species-level is challenging
\end{itemize}}
...
\begin{itemize}
  ...
  \item Fuzzy coding is one option to express uncertainty, but it's not only used for variability in traits within taxononomic groups above species-level. It's also used for variation in traits during the development of an organism. So it's not really clear for fuzzy coded taxa on family-level what uncertainty is expressed?
  \item Are other measures of uncertainty thinkable, e.g. based on SD or Variance 
\end{itemize}
}


\textit{\color{blue} Further ideas what could be included: 
\begin{itemize}
  \item Which traits are more consistent
  \item How have the aggregation and assignments performed when looking back at the original data?
  
  % eigenes Thema/abschnitt - sicherlich nicht umspannen - wäre eher so am ende auch, was an Forschung noch notwendig ist - quality evaluation of traits als Thema..
  \item hat do we really know because someone has watched the animals and published their behaviour; what is extrapolated knowledge? Where does the classification come from?
  \item Where does the classification come from?
  \begin{itemize}
    \item  Where does the classification come from? Few sources used in NOA also used in AUS:
    \item AUS: Merrit \& Cummins 1978/1984 for feeding mode for a few genera and families 
    \item     NOA: Merritt, Richard W., Cummins, Berg 2008 -> but not clear if for feeding mode as well; according to trait description "Cummins, K. W. Trophic Relations of Aquatic Insects" was used for feeding mode
  \end{itemize}
\end{itemize}
}
...
% R: Schwer zu verstehen - versuche positive Sätze nicht negative - also nicht "did not respond" eher "without response" or eben wirklcih positiv "traits that responded only in the original analysis but were non-responsive in the reanalysis
...
\textit{\color{blue}
\begin{itemize}
  \item Resp gills: Original data has a lot of zeros, which are not present in the harmonized \& aggregated datasets (see Table S2)
  \begin{itemize}
    \item In the analysis with the harmonized dataset resp gills is not responding to the salinity gradient
    \item Aggregated data look similar to the harmonized ones, but resp gills is here one of the traits
    responding to conductivity!
  \end{itemize} 
  \item Why did some aggregated datasets promote life cycle duration traits and others not
  \begin{itemize}
    \item Results using trait data obtained when aggregating with stepwise\_agg (median) and stepwise\_agg (mean) indicate that life cycle duration traits are not characterising upstream or downstream sites.
    \item Results using trait data obtained with the other aggregation methods indicate that life cycle duration traits characterise upstream or downstream sites
    \item However: Mean, median and SD of these traits are all relatively similar across all aggregated datasets (see Table S2). 
  \end{itemize}
  \item Extrapolate to "large" scale studies, i.e. studies with a stronger gradient?
  
 % R: Ja, müsste aber wenn an den Anfang wo es um generell um Traits geht, siehe auch email. Könnte mir vorstellen 4 Abschnitte zu machen: 1. Trait Database - Unterschiede zwischen Regionen und Trait Genauigkeit - Was kann man machen - Andere Studien zur Trait Harmonisierung usw. 2. + 3. die aktuellen Abschnitte (obwohl sie auch etwas ähnliche Themen beinhalten - geht ja immer um was ähnliches - wie gut sind die Aggregationsmethoden - könnte man vielleicht auch zusammenfassen 4. Ausblick und offene Fragen für zukünftige Studien
  
  \item Discussion point BK: It is intersting that Australia has fewer species and genus than NOA and EU (not suprisngly since the datasets in Oz were mostly delveoped at family level) but more families than these other regions. I wonder if this indicates that Family richness is really higher in Australia. Perhaps it might be as Australia has a wider range of evolutionary histories. This would be worth considering in the discussion 
\end{itemize}
}
