...
1. Invertebrate traits are increasingly used in ecological studies because they provide a mechanistic understanding of species-environment relationships and are suitable for large scale analysis due to their consistent response to environmental gradients. The development of invertebrate trait databases for different continents in recent years should have created impetus for trait-based research, especially on large scales. However, a number of data-related problems hamper the work with trait-based approaches. For example, standardised trait definitions are lacking and coding of traits varies across trait databases impeding trait data synthesis. Another issue is different taxonomic resolution between observational taxonomic data and trait databases, leading to the need to aggregate traits. 
% Aim of our study?

2. In this study, we describe discrepancies in trait definitions of trait databases from Europe, North America, New Zealand and Australia for traits frequently used in ecological studies. Based on these databases we established 4 novel trait datasets by harmonising 7 commonly used grouping features. Two of these datasets were used to evaluate different trait aggregation methods by comparing aggregated traits with assigned traits by experts at family-level. Furthermore, we explored the effect of grouping feature harmonisation and trait aggregation by re-analysing a study on the effect of anthropogenic salinisation on biological traits using harmonised grouping features and aggregated traits.

3. We found that investigated trait databases are distinct with regard to how grouping features are differentiated into traits, the scope of the grouping feature definitions, how traits are defined, and the coding to describe traits. Comparison of aggregated and assigned traits showed that aggregated traits are generally in conclusion with expert assignments, especially approaches using the median. Grouping feature harmonisation and trait aggregation had a minor impact in the re-analysis, although averaging effects can occur due to harmonisation diminishing the signal of a trait.

4. By outlining discrepancies in trait definitions we hope to motivate the development of a standardized terminology for invertebrate traits. When trait aggregation is needed, our results provide guidance under which circumstances the choice of a trait aggregation method becomes important. 
