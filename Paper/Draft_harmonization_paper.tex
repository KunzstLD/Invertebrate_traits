...
% TODO: Check naming of tables! 
Thus, the signal in the original data for the trait shredder has presumably been weakened by the harmonization, which might be the reason that shredders showed a less strong reaction to the salinity gradient. 

% Resp gil complicated: Original data has a lot of zeros, not in the harmonized & aggregated datasets
% Aggregated data look similar to the harmonized ones, but resp gil is here one of the "extreme" traits 
% on the conductivity axis

% Why did some aggregation methods promote life cycle duration and others not?
% Mean, median and sd's are all relatively similar across tested datasets for these traits
