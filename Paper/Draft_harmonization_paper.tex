...
Although comprehensive invertebrate trait databases have been developed for several regions in the past, data synthesis is difficult due to discrepancies in trait definitions. By providing an overview of definition discrepancies we hope to set a starting point for the development of standardized trait definitions through which trait databases can be linked. Standardized terminology will facilitate comparable trait analysis across large scales. 
% - Variation in taxonomic coverage 
We showed that trait aggregation is often in conclusion with expert assignments and specifically approaches using the median. ... weighting trait information based on the taxa present in the trait databases exerts a minor influence on the aggregated trait affinities. Which method the objectively better measure is cannot be answered, but when trait aggregation is necessary we showed 1) that highly variable traits should be treated carefully, 2) coding of traits should be considered, and 3) taxa where a few genera have a disproportional high number of species trait information .... If researchers need to aggregate traits, a measure of trait dispersion should be reported to indicate the uncertainty of the aggregated estimate. % Question for future research?
% Fuzzy coding and uncertainty?
